...
fn parseCNumLit(c: *Context, m: *MacroCtx) ParseError!Node {
    ...
    switch (m.list[m.i].id) {
        ...
        .FloatLiteral => |suffix| {
            ...
            if (lit_bytes.len >= 2 and std.ascii.is_eq_woc(std.ascii.as(lit_bytes[0..2]).?, std.ascii.as("0x").?)) {
                if (mem.indexOfScalar(u8, lit_bytes, '.')) |dot_index| {
                    if (dot_index == 2) {
                        lit_bytes = try std.fmt.allocPrint(c.arena, "0x0{s}", .{lit_bytes[2..]});
                    } else if (dot_index + 1 == lit_bytes.len or !std.ascii.Char.as(lit_bytes[dot_index + 1]).?.is_hex()) {
                ...
                } else if (dot_index + 1 == lit_bytes.len or !std.ascii.Char.as(lit_bytes[dot_index + 1]).?.is_dec()) {
                    // If the literal lacks a digit after the `.`, we need to
